...
var (
	...
	VALUES(?,?)`
	QueryGetAllTurnos = `SELECT ID, DNI_paciente,Matricula_odontologo,Fecha_Hora,Descripcion 
	FROM my_db.turno`
	...
	QueryUpdateTurno = `UPDATE my_db.turno SET DNI_paciente = ?, Matricula_odontologo = ?,  Fecha_Hora = ?, Descripcion = ?
	WHERE ID = ?`
